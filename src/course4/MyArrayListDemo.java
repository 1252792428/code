 ...
 * @param <E>
 */
public class MyArrayListDemo<E> {
    ...
    */
    ...
    //实现三个构造方法
    public MyArrayListDemo(int n) {//根据传入值设置数组的初始容量
        if (n > 0) {
            E[] es = (E[])new Object[n];//创建容量为n的数组
			this.elements = es;
            this.size=0;//一开始数组里个数为0
        } else {
            throw new IllegalArgumentException("数组的长度(容量)不能为负数");
        }
    }
     public MyArrayListDemo() {
        this.elements = (E[])new Object[10];//设置数组的默认初始容量为10
         this.size=0;
    }
     public MyArrayListDemo(Collection<Object> c) {
        elements = (E[])c.toArray();//将集合转换为数组
     ...
     public void ensureCapacity(int newCapacity){
         ...
         if(newCapacity>curr){
            elements=Arrays.copyOf(elements, newCapacity);//数组拷贝的方法
         }
     }
    ...
    //移除某个 范围内 的元素(不包括end）
     ...
     public static void main(String[] args) {
        MyArrayListDemo<Integer> testMyArrayList=new MyArrayListDemo<Integer>();
